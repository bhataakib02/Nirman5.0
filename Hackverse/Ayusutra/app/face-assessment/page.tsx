...
export default function FaceAssessment() {
  ...
  useEffect(() => {
    if (user && step === 'instructions') {
      setTimeout(() => {
        speakText("Welcome to the face assessment. Please follow the instructions to capture a clear image of your face for analysis.");
      }, 1000);
    }
  }, [user, step, speakText]);
  ...
  const generateSkinAnalysis = () => {
    ...
    const skinTypes = [
      ...
      {
        skinType: "Oily",
        complexion: "Medium to Tan",
        texture: "Slightly Oily with Visible Pores",
        recommendations: [
          "Use oil-free, salicylic acid cleansers",
      ...
      },
      {
        ...
        complexion: "Medium",
        texture: "Oily T-zone, Normal Cheeks",
        recommendations: [
          "Use different products for different areas",
          "Apply oil-free products on T-zone",
          "Use hydrating products on cheeks",
          "Use gentle exfoliants 2-3 times per week"
        ]
      },
      {
        skinType: "Sensitive",
        ...
        recommendations: [
          ...
          "Apply gentle, soothing moisturizers",
  ...
  const processImage = async () => {
    ...
    speakText("Face image captured successfully. Analyzing your skin type and complexion...");
    
    // Simulate analysis delay
    setTimeout(() => {
      const analysis = generateSkinAnalysis();
      setSkinAnalysis(analysis);
      
      // Update assessment data with face image and skin analysis
      const existingData = localStorage.getItem('assessmentData');
      if (existingData) {
        const assessmentData = JSON.parse(existingData);
        assessmentData.faceImage = capturedImage;
        assessmentData.skinAnalysis = analysis;
        localStorage.setItem('assessmentData', JSON.stringify(assessmentData));
        
        // Create booking data for payment page
        const bookingData = {
          therapyId: 'ayurvedic-assessment',
          therapyName: 'Complete Ayurvedic Assessment',
          clinicId: 1,
        ...
        const assessmentData = {
          ...
          height: '165',
          weight: '55',
          symptoms: ['Headaches', 'Fatigue', 'Digestive issues'],
          stressLevel: 3,
          tongueImage: null,
          hairImage: null,
      ...
      speakText(`Based on the analysis, your skin type is ${analysis.skinType}. Your complexion is ${analysis.complexion} with ${analysis.texture} texture. All assessments are now complete. I will now redirect you to your dashboard.`);
  ...
  const faceAnalysisInstructions = [
    {
    ...
    },
    {
      ...
      title: "Clear View",
      description: "Position your face clearly in the frame, looking directly at the camera"
    },
    {
              ...
              <CardHeader className="text-center space-y-4">
                ...
                <div>
                  ...
                  <CardDescription className="text-lg text-emerald-200 max-w-2xl mx-auto">
                    In Ayurveda, facial features reveal your constitution and health status. 
                    Our AI will analyze your face to provide personalized health recommendations.
                  </CardDescription>
                </div>
              </CardHeader>
              ...
              <CardContent className="space-y-8">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    {faceAnalysisInstructions.map((instruction, index) => (
                ...
                <input
                  ...
                  className="hidden"
                  aria-label="Upload face image for analysis"
                  title="Upload face image for analysis"
                />
                ...
                <div className="flex justify-center">
                  <Button
                    variant="outline"
                    onClick={() => router.push('/hair-assessment')}
                    className="flex items-center space-x-2 border-amber-400 text-amber-200 hover:bg-amber-500/20"
                  >
                    <ArrowLeft className="h-4 w-4" />
                    <span>Back to Hair Assessment</span>
                  </Button>
                </div>
              </CardContent>
            </Card>
      ...
      <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-teal-800 to-green-900 flex items-center justify-center p-4">
        <Card className="w-full max-w-2xl bg-black/20 backdrop-blur-lg border border-amber-400/30 shadow-2xl">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-emerald-200">
              Is this image clear and well-lit? We'll analyze it for facial health insights.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            {capturedImage && (
          ...
          <CardContent className="text-center py-12 space-y-6">
            ...
            <div>
              ...
              <p className="text-emerald-200">
                Our AI is examining facial features, skin condition, and overall health indicators.
              </p>
            </div>
            <div className="space-y-2 text-sm text-emerald-300">
              <div>🔍 Analyzing facial features...</div>
              <div>📊 Evaluating skin condition...</div>
            ...
            {skinAnalysis && (
              <div className="mt-8 p-6 bg-amber-500/10 border border-amber-400/30 rounded-xl">
                <h4 className="text-lg font-semibold text-amber-200 mb-4">Skin Analysis Results</h4>
                <div className="space-y-3 text-left">
                  <div>
                    <span className="text-amber-300 font-medium">Skin Type:</span>
                    <span className="text-amber-100 ml-2">{skinAnalysis.skinType}</span>
                  </div>
                  <div>
                    <span className="text-amber-300 font-medium">Complexion:</span>
                    <span className="text-amber-100 ml-2">{skinAnalysis.complexion}</span>
                  </div>
                  <div>
                    <span className="text-amber-300 font-medium">Texture:</span>
                    <span className="text-amber-100 ml-2">{skinAnalysis.texture}</span>
                  </div>
                </div>
                <div className="mt-4">
                  <h5 className="text-amber-300 font-medium mb-2">Recommendations:</h5>
