...
from .custom_dataset import TongueCSVData
...
def get_dataloaders(val_ratio: float = 0.2, batch_size: int = 16):
    """
    Create train/val dataloaders using labels.csv + .bmp images.
    """
    ...
    full_dataset = TongueCSVData(csv_path=CSV_PATH, images_dir=DATASET_DIR, transform=train_transform)
    class_names = full_dataset.class_names       # e.g. ["kapha","pitta","vata"]
    n_total = len(full_dataset)

    if n_total == 0:
        raise RuntimeError("Dataset is empty. Check labels.csv and image paths.")
    ...
    # For validation, remove strong augmentation
...
def build_model(num_classes: int, freeze_backbone: bool = True) -> nn.Module:
    """
    EfficientNet-B0 for dosha classification (Vata/Pitta/Kapha).
    """
            ...
            torch.save(
                {
                    ...
                    "class_names": class_names,  # <--- used by backend
                },
                MODEL_PATH,
            )
